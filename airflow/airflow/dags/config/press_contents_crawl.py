...
# 파일명에 'press'를 명시하여 다른 모듈과 충돌 방지
PROCESSED_LINKS_FILE = os.path.join(DATA_PATH, "processed_press_contents_links.json")

def get_output_filenames():
    today_str = datetime.today().strftime("%Y%m%d")
    output_csv = os.path.join(DATA_PATH, f"fed_press_releases_contents_{today_str}.csv")
    output_json = os.path.join(DATA_PATH, f"fed_press_releases_contents_{today_str}.json")
    output_parquet = os.path.join(DATA_PATH, f"fed_press_releases_contents_{today_str}.parquet")
...
def extract_text_from_pdf_url(pdf_url):
    try:
        ...
        logging.warning(f"⚠️ PDF 텍스트 추출 오류 ({pdf_url}): {e}")
        return None
...
def scrape_press_release(url):
    try:
        ...
        pdf_texts = [extract_text_from_pdf_url(pdf) for pdf in pdf_links]
        pdf_combined_text = "\n".join([pdf for pdf in pdf_texts if pdf])

        if pdf_combined_text:
            text_content = text_content + "\n" + pdf_combined_text if text_content else pdf_combined_text
...
def update_contents(s3_prefix):
    ...
    if new_records.empty:
        logging.info("새로운 콘텐츠 크롤링 대상이 없습니다.")
        return

    new_links = new_records["link"].tolist()
    logging.info(f"새로운 콘텐츠 크롤링 대상: {len(new_links)}개")
    ...
    if results:
        ...
        logging.info(f"✅ 신규 콘텐츠 크롤링 완료! {len(results)}개 데이터 저장됨: {output_csv}")
